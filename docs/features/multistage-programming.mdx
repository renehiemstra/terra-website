...
Multistage programming is a cornerstone of Terra's design, enabling developers to write code that generates and specializes other code at runtime or compile-time. By treating Terra's functions, types, variables, and expressions as first-class values in Lua, multistage programming allows for dynamic code construction, optimization, and specialization. This paradigm, rooted in the interplay between Lua's flexibility and Terra's high-performance compilation, empowers developers to create efficient, adaptive programs with minimal boilerplate.

## Dynamic Code Generation
Multistage programming allows Terra to generate code dynamically, tailoring it to specific use cases. For example, you can create functions optimized for particular input sizes, hardware targets, or performance constraints. This is particularly powerful for tasks like loop unrolling, constant folding, or generating SIMD-optimized routines, all driven by Lua's logic.


### Example: loop unrolling in polynomial evaluation
We explore loop unrolling in Terra for evaluating a cubic polynomial $ p(x) = a + b \cdot x + c \cdot x^2 + d \cdot x^3 $, where coefficients ae stored in a struct array (`coeffs[0] = a`, `coeffs[1] = b`,  `coeffs[2] = c`,  `coeffs[3] = d`.). Horner's method rewrites the polynomial as $ a + x \cdot (b + x \cdot (c + x \cdot d)) $ to minimize multiplications and additions, improving numerical stability and performance.
...
View the generated assembly with `:disas()` (with -O2 on an AMD):
```
...
We can further optimize the code by fusing the addition and multiplication into one operation. Here's the optimized code integrated into the Polynomial factory (replace the original eval method and add the intrinsic definition):
